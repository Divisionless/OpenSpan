...
| 8 | **Restart into the stable build** (Doug: "Ensure that windows starts up entirely into this new stable build -- we will test soon"). Pre-steps by me: app tree down → `vm_off_gently.py` (guest off, no captures held) → confirm `list usbhost` shows no `Captured`. Then Doug restarts Windows. After sign-in the HKCU Run key launches **EsotericOS.exe (7aab8cb7)** and **EsotericOS Shell** (Cairo fork, theme + Files as file manager); custody is a registry binding and comes with boot. Post-checks by me: `radio_custody.py audit` (three CUSTODY before the VM captures, then all three held), `status.json` (vm up, daemon, portal restored), Cairo windows present, z-order (app above nothing but Progman) | proves the whole state survives boot; first restart since custody | 30 s pre + restart + 3 min verify | Run key verified 16:4x; dead `OpenSpan` Run value removed | `bake-in.ps1 -Undo`; `autostart-shell.ps1 -Undo`; `return <id> --apply` per radio |
