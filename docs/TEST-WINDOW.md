...
| 9 | **First boot with the fork AS the shell** (set 16 Aug, applies next sign-in). Cold restart from the Forge → Windows starts **CairoDesktop.exe as the shell** (no Explorer), which runs StartupRunner → launches **EsotericOS.exe** (displays + Bluetooth stand-down + VM) and OneDrive. Watch for: a usable desktop (taskbar/menu/tray), the app on the desktop, **the Forge (Electron) behaving** (v3.114 post-hoc), no Cairo tour, no Software-Update pop. If a sign-in comes up wrong: Ctrl+Shift+Esc → Run `explorer.exe`, then `shell\tools\restore-explorer-shell.ps1` + `autostart-shell.ps1 -Undo`. | the actual takeover; owns the boundary from boot | restart + 5 min verify | shell built (fd6b168d), HKCU Shell set, app Run entry intact | Task Manager → explorer.exe; restore-explorer-shell.ps1; autostart-shell.ps1 -Undo |
| 10 | **Law 10 verification — appended to Doug's next restart** (his word, 2026-08-27: "I will not be running scripts in such a fashion, append it to my next restart"). While the desk is already down for the restart, I run `win\run_all_tests.py` — the eleven suites that construct a real `Tk()` root and so cannot run during a live session, chiefly `test_single_page.py` (the inverted wheel contract), `test_bt_volume.py` and `test_pair_flow.py` (both drive `_apply_rows`, the new `fit_tree_height` path). Watch for on first sign-in after: the Bluetooth device list and Console **have no scrollbars of their own**, the wheel scrolls the page from any pixel, the device list **grows as a scan lands** (it will now shift rows under the cursor — deliberate, see DEVLOG), and the Console no longer jumps to its tail on new output. | law 10 changed a tested contract; the static half is green but the widget-fitting paths need a real Tk root | tests ~2 min, verify ~3 min | law-10 diff in the working tree (`win\openspan.py`, `win\test_single_page.py`); static half already 33/33 green | `git checkout -- win/openspan.py win/test_single_page.py` — the change is uncommitted and reverts whole; no build, no installer, no registry, nothing armed on disk |
| 8 | **Cold restart into the stable build — Doug's hands, Doug's schedule** ("restart when i am at a stopping point in the forge IDE -- and then for it to come up with Esoteric OS loaded and nothing else except my normal startup stuff"). No pre-steps: he restarts cold from the Forge. Designed for: `start_vm_clean()` discards the VM state VirtualBox saves at host shutdown and cold-boots the guest; custody is a registry binding; the HKCU Run key launches **EsotericOS.exe (7aab8cb7)** and **EsotericOS Shell** (Cairo fork: theme, menu, status widget, Files as file manager); WinSparkle's upstream update pop-up switched off; dead `OpenSpan` Run value removed; both entries enabled in Startup apps. Post-checks by me after sign-in: `radio_custody.py audit` (all three held), `status.json` (vm up, daemon, portal restored), Cairo windows present, nothing else of ours on screen | proves the whole state survives boot; first restart since custody | restart + 3 min verify | done 16:5x | `bake-in.ps1 -Undo`; `autostart-shell.ps1 -Undo`; `return <id> --apply` per radio |
