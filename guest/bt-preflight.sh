...
# HID lanes first releases the GATT objects they registered.
systemctl stop openspanble openspanble-mac >/dev/null 2>&1
...
for _ in $(seq 15); do
    sleep 1
    if probe; then
        # bring the lanes back up now that the bus answers again
        systemctl start openspanble openspanble-mac >/dev/null 2>&1
        echo "BT_RECOVERED"
        exit 0
    fi
done
